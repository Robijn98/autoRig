...
class eye:
    ...
    def eye_guides(self):
        grp = f'{self.side}_{EYE}_{GUIDE}_{GROUP}'
        guides = []
    ...
    def create_minor_eye_jnts(self):
        main_grp = cmds.group(empty=True, name=f'{self.side}_{EYE}_minor_{GROUP}')
        cmds.select(cl=True)

        minor_joints = []
        guides = eye_guides()

        mid_eye_guide = f'{self.side}_{CENTER}_{EYE}_{GUIDE}'
        ...
        #aim_jnt
        aim_guide = f'{self.side}_{EYE}Aim_{GUIDE}'
    ...
    def eye_connection(self, influenceProcent=0.2):
        aim_jnt = f'{self.side}_{EYE}Aim_{JOINT}'
        aimed_jnt = f'{self.side}_{CENTER}_{EYE}_{JOINT}'
        cmds.aimConstraint(aim_jnt, aimed_jnt, mo=False, aim=(0, 0, 1))

        #create ctrl
        ctrl_grp, ctrl = create_temp_ctrl(f'{self.side}_{EYE}_{self.CONTROL}', lock=['sx', 'sy', 'sz', 'rx', 'ry', 'rz'])
        ...
        mid_jnt_upper = f'{self.side}_{EYE}upper_{mid_jnt}_{JOINT}'
        mid_jnt_lower = f'{self.side}_{EYE}lower_{mid_jnt}_{JOINT}'
